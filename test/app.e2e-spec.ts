...
describe('AppController (e2e)', () => {
  ...
  it('Registar um novo usuário', async () => {
  ...
  it('Obter os dados do usuário logado', async () => {
    ...
    expect(response.statusCode).toEqual(201);
    expect(typeof response.body.tokenPayload.id).toEqual('number');
    expect(response.body.tokenPayload.role).toEqual(Role.User);
  });

  it('Registar um novo usuário como administrador', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/register')
      .send({
        ...authRegisterDTO,
  ...
  it('Validar se a função do novo usuário ainda é User', async () => {
    ...
    userId = response.body.tokenPayload.id;
  });
  ...
  it('Alterando o usuário para a função adminstrador', async () => {
